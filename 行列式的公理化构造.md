...
又根据 $(1.14)$ , 有 
$$\mathcal{P}(F_{i,j}(\lambda) \, A) = \mathcal{P}(A) \tag{1.15}$$
...
$$
\begin{align}
A' & = (A_{(1)}, \cdots , A_{(i - 1)}, G, A_{(i + 1)}, \cdots , A_{(n)}) \\
A'' & = (A_{(1)}, \cdots , A_{(i - 1)}, H, A_{(i + 1)}, \cdots , A_{(n)})
\end{align}
$$
向量组 $\{A_{(1)}, \cdots , A_{(i - 1)}, G, H, A_{(i + 1)}, \cdots, A_{(n)}\}$ , 由 $n + 1$ 个向量组成, 
因此它必定是线性相关的.
如果 ${A_{(1)}, \cdot , A_{(i - 1)}, A_{(i + 1)}, \cdots , A_{(n)}}$ 是线性相关的，
...
那么 $A'$ 和 $A''$ 中至少有一个矩阵是的行向量是线性相关的. 不失一般性的
