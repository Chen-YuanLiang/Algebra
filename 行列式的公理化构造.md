...
$$
A' = (\cdots , A_{(s - 1)}, A_{(t)}, A_{(s)}, \cdots, A_{(t - 1)}, A_{(s)}, A_{(t + 1)}, \cdots) = F_{s, t} \ A
$$
则
$$
\mathcal{P}(A') =  \mathcal{D}(^{t}(F_{s, t}A)) = \mathcal{D}(^{t}A \ ^{t}F_{s,t}) = \mathcal{D}(^{t}A) \cdot \mathcal{D}(^{t}F_{s,t}) \tag{1.11}
$$
因为 $F_{s,t} = {^{t}F}_{(s, t)}$, 因此 $\mathcal{D}({^{t}F}_{(s, t)}) = \mathcal{D}(F_{(s, t)}) = -1$ , 带入 $(1.11)$ 得
$$
\mathcal{P}(A') = (-1) \mathcal{D}(A) = (-1) \mathcal{P}(A) \tag{1.22}
$$
因此 $\mathcal{P}(A)$ 对于 $A$ 的行具有斜对称性. </br>
设 $A_{(i)} = B + C, \,(B)$ 的第 $i$ 行等于 $A'_{()}$
...
$$
\begin{align}
\lambda \mathcal{D}(\cdots , A_{(i)}, \cdots , A_{(j)}, \cdots) & = \mathcal{D}(\cdots , A_{(i)}, \cdots , \lambda A_{(j)}, \cdots) \\
& = \lambda \mathcal{D}(\cdots , A_{(i)} + \lambda A_{(j)}, \cdots , \lambda A_{(j)}, \cdots)
\end{align}
$$
...
那么就会有两种情况:一是$A_{(i)}', A_{(i)}''$ 都不是 $(2.3)$ 中除自己外的向量的线性组合; 
二是$A_{(i)}', A_{(i)}''$至少有一个是 $(2.3)$ 中除自己外的向量的线性组合</br>
如果 $A_{(i)}' , A_{(i)}''$ 都不是 $(2.3)$ 中除自己外的向量的线性组合, 那么
$$
\{A_{(1)}, \cdots , A_{(i - 1)}, A_{(i + 1)} , \cdots , A_{(n)}\} \tag{2.4}
$$
是线性相关的, 因此根据前面所得结论有:
$$
\begin{align}
...
如果 $A_{(i)}' , A_{(i)}''$ 至少有一个是 $(2.3)$ 中除自己外的向量的线性组合, 
不妨设该向量是 $A_{(i)}'$, 
即 
$$
